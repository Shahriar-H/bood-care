  ...
  const signupHandle = ()=>{
    try {
      if(formData?.password?.length<6){
        return Alert.alert("Warning","Password Must be at least 6 char.")
      }
      ...
      fetch(api_url+"/insert-item",{
        ...
        body: JSON.stringify({data:{mobile:formData?.mobile,password:formData?.password},table:"users"})
      })
      .then((res)=>res.json())
      .then(async(res)=>{
        console.log(res);
        ...
        await storeIntroData({...formData,_id:res?.result?.insertedId})
      
        loginFun({...formData,_id:res?.result?.insertedId})
        setformData({
          mobile:"",
          password:''
        })
  ...
  return (
    <View className="flex-1 justify-center px-6 bg-white">
      ...
      <TouchableOpacity disabled={isLoading} onPress={signupHandle} className="w-full py-2 bg-red-500 rounded-lg mb-6">
        <Text className="text-center text-white text-lg font-bold">{isLoading?"Signing up":'Signup'}</Text>
      </TouchableOpacity>
